...
class GUI(QMainWindow):
    ...
    def update_fig_power(self):
        ...
        for k in self.main.cells_package.keys():
            if not self.main.cells_package[k]['data_file'].df_full_params.empty:
                if df_power.empty:
                    df_power['UNIX_Timestamp(s)'] = self.main.cells_package[k]['data_file'].df_full_params[
                        'UNIX_Timestamp(s)']
                    df_power['Date_Timestamp'] = self.main.cells_package[k]['data_file'].df_full_params[
                        'Date_Timestamp']
                    df_power['Power'] = self.main.cells_package[k]['data_file'].df_full_params['Power']
                else:
                    df_instance = pd.merge(df_power, self.main.cells_package[k]['data_file'].df_full_params[
                        ['UNIX_Timestamp(s)', 'Date_Timestamp', 'Power']], on='UNIX_Timestamp(s)', how='inner')
    ...
    def update_cell_parameters(self, cell):
        if cell != None:
            ...
            getattr(self.ui, f'PCEStr{index + 1}').setText(
                f'{round(self.main.cells_package[cell]["cell"].pce * 100, 1)}')
        ...
        while (i < cycle or loop_bool) and not self.worker[0].checkstop:
            ...
            if i < cycle or loop_bool:
                if remaining_time != None:
                    ...
                    time.sleep(1)
                    while (self.ui.NextMeasurementTimeIntervalLabel.text() != 'Measuring...') and not self.worker[
                        0].checkstop:
                        time.sleep(0.1)
        if remaining_time != None:
    ...
    def update_tab_stability(self):
        for param in ['isc', 'voc', 'ff', 'pce']:
            ...
            if not self.cell_package['data_file'].df_full_params.empty:
                fig.axes.plot_date(pd.to_datetime(self.cell_package['data_file'].df_full_params['Date_Timestamp']),
                                   self.cell_package['data_file'].df_full_params[df_full_params_column], xdate=True,
